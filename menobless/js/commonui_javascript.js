function execute_comment(){
    ...
    let blank_a_tags=document.getElementsByTagName("a");
    for(let b=0; b<blank_a_tags.length; b++){
        let item=blank_a_tags[b];
    ...
    function commonui_javascript_event_initialize(){
       ...
       //모두동의,특정 그룹포함하는 mother개념 라디오,체크박스 체크시에 하위관련그룹체크박스들 모두 체크/언체크
        ...
        for(let s=0;s<styleEffectTrigger.length; s++){
            ...
            let trigger_events=JSON.parse(item.getAttribute("triggerevents"));

            for(let events in trigger_events){
