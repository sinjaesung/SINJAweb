    ...
    return function execution_function(flag_object,target_item,styleProperty,style_originalval,style_nextval,units,anim_duration){
        return new Promise(function(resolve,reject){
            ...
            //for(let e=0; e<execution_queue.length; e++){
               ...
               let clear_filtered=execution_queue.filter((element)=> ( (element['group']==queue_info['group'] && element['object']==queue_info['object']) || (element['group'].indexOf("mouseleave")==-1 && element['group'].indexOf("mouseenter")==-1))?(true):(clearInterval(element['functionid'])));//현재 그룹 or object인것(현재 object>eventType인것만 남김) 나머지 모두 제거!(현재 focusing object->focusing event가 아닌것들 큐anim는 모두 제거) 대신 mouseleave형태의 타입들에 한해서.모두 css reset복귀 처리(instant형태로 진행) mouseenter타입이 아닌것들이거나, 현재 foucising된것들만 남기고, 그외의것들 ouseenter타입이거나(focsing아닌 mouseEnter,mouseleave타입들 모두 제거, 그외의것들은 이벤ㅌ타입들은 냅둔다.)
