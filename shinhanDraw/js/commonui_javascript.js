function execute_comment(){
    ...
    function commonui_javascript_event_initialize(){
       ...
       //모두동의,특정 그룹포함하는 mother개념 라디오,체크박스 체크시에 하위관련그룹체크박스들 모두 체크/언체크
        ...
        let effect_triggerss=document.getElementsByClassName("javascript_StyleEffectTrigger2");
        for(let s=0;s<effect_triggerss.length; s++){
            let item=effect_triggerss[s];
            let target_data=JSON.parse(item.getAttribute("targetdata"));
            let eventtypes=target_data['event_per_effects'];
            console.log('targetadatsas:',target_data);
            for(let e in eventtypes){
                item.addEventListener(e,styleEffect_adapt_trigger2,false);
            }
        }
    ...
    function styleEffect_adapt_trigger2(event){
        console.log("=====================================javascript_styleEffectTrigger2실행===============================",event.target,event.type);

        let target=event.target;
        let event_type=event.type;

        if(target.className.indexOf("javascript_StyleEffectTrigger2")==-1){
            target=parentTargetfind(target,"javascript_StyleEffectTrigger2");
        }
        console.log("effect adapt tirgger find target realss:",target);
        ...
        let eventper_targets=event_per_effects[event_type];        
        console.log("발생 이벤트 및 eventper_targets:",event_type,eventper_targets);
        
        //let effectTarget=matching_effects['targetindexes'];//해당 이펙트에 관련된 타깃들 Array(targetindex Stringss array)
        //let animate_parameters=matching_effects['animate_parameters'];//aniamte changte paramtersss object

        for(let s=0; s<eventper_targets.length; s++){
            let target_local=eventper_targets[s];//target option objecdtss 프로퍼티명(targetindex),프로퍼티값(객체)

            for(let targetindexes in target_local){
                
                let style_optionItem=target_local[targetindexes];
                ...
                if(typeof(style_optionItem)=='object'){
                    ...
                    let animaftercallback=style_optionItem['animaftercallback'];

                    console.log("target_local and local effectss",targetindexes,style_object,style_object_instant,duration,animaftercallback);
                    ...
                    if(style_object){
                        ...
                        $(`[targetindex='${targetindexes}']`).animate(style_object,duration,function(){
                            console.log("callback fucnton실행 event_type:",event_type,animaftercallback);
                            if(event_type==='mouseleave'){
                                $(`[targetindex='${targetindexes}']`).removeAttr('style');
                            }
                            if(animaftercallback){
                                $(`[targetindex='${targetindexes}']`).css(animaftercallback);
                            }
                        })
                    }
                    
                    
                }else if(style_optionItem.indexOf("inlinestyle_reset")!=-1){
                    console.log("inlineStyle 초기화!!!");
                    $(`[targetindex='${targetindexes}']`).removeAttr('style');
                }
                             
            }
        }
        console.log("===========================effect adapt trigger tirgterssJqueryAnimate End===============================");
